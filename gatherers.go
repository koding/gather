package main

import (
    "bytes"
    "compress/gzip"
    "fmt"
    "io"
)

func bindata_read(data []byte, name string) ([]byte, error) {
	gz, err := gzip.NewReader(bytes.NewBuffer(data))
	if err != nil {
		return nil, fmt.Errorf("Read %q: %v", name, err)
	}

	var buf bytes.Buffer
	_, err = io.Copy(&buf, gz)
	gz.Close()

	if err != nil {
		return nil, fmt.Errorf("Read %q: %v", name, err)
	}

	return buf.Bytes(), nil
}

func gatherers_common() ([]byte, error) {
	return bindata_read([]byte{
		,
		},
		"gatherers/common",
	)
}

func gatherers_run_abu() ([]byte, error) {
	return bindata_read([]byte{
		,
		},
		"gatherers/run-abu",
	)
}

func gatherers_run_bac() ([]byte, error) {
	return bindata_read([]byte{
		0x1f, 0x8b, 0x08, 0x00, 0x00, 0x09, 0x6e, 0x88, 0x00, 0xff, 0xca, 0x2f,
		0x2d, 0x29, 0x28, 0x2d, 0x89, 0x2f, 0xcd, 0xcb, 0x49, 0x2d, 0x2e, 0x8e,
		0x4f, 0xcd, 0x2d, 0x28, 0xa9, 0x54, 0x50, 0x4a, 0x4a, 0x2c, 0xce, 0x28,
		0x4a, 0x56, 0xc8, 0xc9, 0xcc, 0x4b, 0x55, 0x48, 0xce, 0x2f, 0xcd, 0x2b,
		0x51, 0x52, 0x50, 0xf1, 0x0b, 0xf5, 0x75, 0x72, 0x0d, 0x52, 0x50, 0xd1,
		0xc8, 0x2b, 0xcd, 0x4d, 0x4a, 0x2d, 0x8a, 0xcf, 0x4f, 0x8b, 0x07, 0xc9,
		0x17, 0x2b, 0xd4, 0xe9, 0xeb, 0x41, 0xd4, 0x6b, 0x72, 0x01, 0x02, 0x00,
		0x00, 0xff, 0xff, 0x5a, 0x72, 0xf5, 0x9e, 0x4d, 0x00, 0x00, 0x00,
		},
		"gatherers/run-bac",
	)
}

func gatherers_run_crd() ([]byte, error) {
	return bindata_read([]byte{
		,
		},
		"gatherers/run-crd",
	)
}

func gatherers_run_dbd() ([]byte, error) {
	return bindata_read([]byte{
		0x1f, 0x8b, 0x08, 0x00, 0x00, 0x09, 0x6e, 0x88, 0x00, 0xff, 0xca, 0xcc,
		0x2b, 0x2e, 0x49, 0xcc, 0xc9, 0x49, 0x4d, 0x51, 0x50, 0x4a, 0x29, 0xca,
		0x2f, 0x48, 0x4a, 0x4d, 0x2c, 0x52, 0x28, 0x2e, 0xce, 0x50, 0x48, 0xce,
		0xc9, 0x4c, 0xcd, 0x2b, 0x51, 0x80, 0x4b, 0x2b, 0x29, 0xc0, 0xa4, 0xb9,
		0x00, 0x01, 0x00, 0x00, 0xff, 0xff, 0xf3, 0x97, 0x69, 0xa5, 0x33, 0x00,
		0x00, 0x00,
		},
		"gatherers/run-dbd",
	)
}

func gatherers_run_dmp() ([]byte, error) {
	return bindata_read([]byte{
		,
		},
		"gatherers/run-dmp",
	)
}

func gatherers_run_dod() ([]byte, error) {
	return bindata_read([]byte{
		0x1f, 0x8b, 0x08, 0x00, 0x00, 0x09, 0x6e, 0x88, 0x00, 0xff, 0xca, 0xcc,
		0x2b, 0x2e, 0x49, 0xcc, 0xc9, 0x49, 0x4d, 0x51, 0x50, 0x4a, 0xc9, 0x4f,
		0xce, 0x4e, 0x2d, 0x52, 0x80, 0x8b, 0x28, 0x29, 0x40, 0x44, 0xb8, 0x00,
		0x01, 0x00, 0x00, 0xff, 0xff, 0x0f, 0x6e, 0x3b, 0x32, 0x24, 0x00, 0x00,
		0x00,
		},
		"gatherers/run-dod",
	)
}

func gatherers_run_esi() ([]byte, error) {
	return bindata_read([]byte{
		0x1f, 0x8b, 0x08, 0x00, 0x00, 0x09, 0x6e, 0x88, 0x00, 0xff, 0x4a, 0xc9,
		0x2c, 0x8a, 0x4f, 0xad, 0xc8, 0x2c, 0x2e, 0x29, 0x56, 0x50, 0xd2, 0x2f,
		0x2d, 0x2e, 0xd2, 0x2f, 0xce, 0x48, 0x2c, 0x4a, 0xd5, 0x4f, 0xcd, 0x49,
		0x2c, 0x2e, 0xc9, 0x4c, 0x2e, 0x4e, 0x4d, 0x2c, 0x4a, 0xce, 0x50, 0x52,
		0x50, 0x42, 0xe1, 0x2b, 0xa4, 0xe5, 0x97, 0xe6, 0xa5, 0x28, 0x71, 0x01,
		0x02, 0x00, 0x00, 0xff, 0xff, 0xcd, 0x10, 0xaf, 0x69, 0x3c, 0x00, 0x00,
		0x00,
		},
		"gatherers/run-esi",
	)
}

func gatherers_run_fic() ([]byte, error) {
	return bindata_read([]byte{
		0x1f, 0x8b, 0x08, 0x00, 0x00, 0x09, 0x6e, 0x88, 0x00, 0xff, 0xca, 0x2f,
		0x2d, 0x29, 0x28, 0x2d, 0x89, 0x2f, 0xcd, 0xcb, 0x49, 0x2d, 0x2e, 0x8e,
		0x4f, 0xcd, 0x2d, 0x28, 0xa9, 0x54, 0x50, 0x4a, 0xcb, 0x2c, 0xce, 0x50,
		0xc8, 0xc9, 0xcc, 0x4b, 0x55, 0x48, 0xce, 0x2f, 0xcd, 0x2b, 0x51, 0x52,
		0x50, 0xf1, 0x0b, 0xf5, 0x75, 0x72, 0x0d, 0x52, 0x50, 0xd1, 0xc8, 0x2b,
		0xcd, 0x4d, 0x4a, 0x2d, 0x8a, 0xcf, 0x4f, 0x8b, 0x07, 0xc9, 0x17, 0x2b,
		0xd4, 0xe9, 0xeb, 0x25, 0xe7, 0xe7, 0xa5, 0x65, 0xa6, 0xeb, 0x83, 0x34,
		0xe9, 0x43, 0xd8, 0x7a, 0x20, 0xb6, 0x26, 0x17, 0x20, 0x00, 0x00, 0xff,
		0xff, 0x55, 0x8c, 0x45, 0x79, 0x5c, 0x00, 0x00, 0x00,
		},
		"gatherers/run-fic",
	)
}

func gatherers_run_hii() ([]byte, error) {
	return bindata_read([]byte{
		0x1f, 0x8b, 0x08, 0x00, 0x00, 0x09, 0x6e, 0x88, 0x00, 0xff, 0xca, 0xcc,
		0x2b, 0x2e, 0x49, 0xcc, 0xc9, 0x49, 0x4d, 0x51, 0x50, 0xca, 0x28, 0xc8,
		0xcc, 0x4b, 0x37, 0x56, 0x80, 0x8b, 0x28, 0x29, 0x40, 0x44, 0xb8, 0x00,
		0x01, 0x00, 0x00, 0xff, 0xff, 0x16, 0xa6, 0xa6, 0xc9, 0x24, 0x00, 0x00,
		0x00,
		},
		"gatherers/run-hii",
	)
}

func gatherers_run_moi() ([]byte, error) {
	return bindata_read([]byte{
		0x1f, 0x8b, 0x08, 0x00, 0x00, 0x09, 0x6e, 0x88, 0x00, 0xff, 0xca, 0xcc,
		0x2b, 0x2e, 0x49, 0xcc, 0xc9, 0x49, 0x4d, 0x51, 0x50, 0xca, 0xcd, 0xcf,
		0x4b, 0xcf, 0x4f, 0x49, 0x52, 0x80, 0x0b, 0x29, 0x29, 0x40, 0x84, 0xb8,
		0x00, 0x01, 0x00, 0x00, 0xff, 0xff, 0x94, 0xde, 0xa3, 0x94, 0x25, 0x00,
		0x00, 0x00,
		},
		"gatherers/run-moi",
	)
}

func gatherers_run_myi() ([]byte, error) {
	return bindata_read([]byte{
		0x1f, 0x8b, 0x08, 0x00, 0x00, 0x09, 0x6e, 0x88, 0x00, 0xff, 0xca, 0xcc,
		0x2b, 0x2e, 0x49, 0xcc, 0xc9, 0x49, 0x4d, 0x51, 0x50, 0xca, 0xad, 0x2c,
		0x2e, 0xcc, 0x51, 0x80, 0x0b, 0x28, 0x29, 0x80, 0x05, 0xb8, 0x00, 0x01,
		0x00, 0x00, 0xff, 0xff, 0xf3, 0xbc, 0xfd, 0xf3, 0x22, 0x00, 0x00, 0x00,
		},
		"gatherers/run-myi",
	)
}

func gatherers_run_nus() ([]byte, error) {
	return bindata_read([]byte{
		,
		},
		"gatherers/run-nus",
	)
}

func gatherers_run_poi() ([]byte, error) {
	return bindata_read([]byte{
		0x1f, 0x8b, 0x08, 0x00, 0x00, 0x09, 0x6e, 0x88, 0x00, 0xff, 0xca, 0xcc,
		0x2b, 0x2e, 0x49, 0xcc, 0xc9, 0x49, 0x4d, 0x51, 0x50, 0x2a, 0xc8, 0x2f,
		0x2e, 0x49, 0x2f, 0x4a, 0x2d, 0x2e, 0xcc, 0x51, 0x80, 0x8b, 0x2a, 0x29,
		0x14, 0x00, 0xf9, 0x5c, 0x80, 0x00, 0x00, 0x00, 0xff, 0xff, 0x22, 0xbd,
		0xc0, 0xb3, 0x26, 0x00, 0x00, 0x00,
		},
		"gatherers/run-poi",
	)
}

func gatherers_run_psi() ([]byte, error) {
	return bindata_read([]byte{
		0x1f, 0x8b, 0x08, 0x00, 0x00, 0x09, 0x6e, 0x88, 0x00, 0xff, 0xca, 0xcc,
		0x2b, 0x2e, 0x49, 0xcc, 0xc9, 0x49, 0x4d, 0x51, 0x50, 0xcf, 0xcb, 0x4d,
		0x2c, 0x50, 0x28, 0xc8, 0x2f, 0x2a, 0x29, 0x4e, 0x4e, 0xcc, 0xcb, 0x4b,
		0x2d, 0x52, 0x80, 0xcb, 0xa9, 0x43, 0x24, 0xd5, 0xb9, 0x90, 0x55, 0x27,
		0x27, 0x96, 0xe0, 0x56, 0x0d, 0x94, 0x44, 0x51, 0x5d, 0x90, 0x07, 0x52,
		0x86, 0x53, 0x3d, 0x44, 0x1a, 0x45, 0x47, 0x76, 0x5e, 0x7e, 0x72, 0x36,
		0x50, 0x19, 0x2e, 0x2d, 0x50, 0x79, 0x54, 0x37, 0x25, 0x16, 0xe3, 0x71,
		0x13, 0x50, 0x52, 0x9d, 0x0b, 0x10, 0x00, 0x00, 0xff, 0xff, 0x30, 0xf1,
		0xb0, 0xe7, 0xf0, 0x00, 0x00, 0x00,
		},
		"gatherers/run-psi",
	)
}

func gatherers_run_sii() ([]byte, error) {
	return bindata_read([]byte{
		0x1f, 0x8b, 0x08, 0x00, 0x00, 0x09, 0x6e, 0x88, 0x00, 0xff, 0xca, 0xcc,
		0x2b, 0x2e, 0x49, 0xcc, 0xc9, 0x49, 0x4d, 0x51, 0x50, 0x2a, 0x2e, 0xcc,
		0xc9, 0x2c, 0x49, 0x55, 0x80, 0x8b, 0x28, 0x29, 0x40, 0x44, 0xb8, 0x00,
		0x01, 0x00, 0x00, 0xff, 0xff, 0xf6, 0xf9, 0x50, 0x9f, 0x24, 0x00, 0x00,
		0x00,
		},
		"gatherers/run-sii",
	)
}

func gatherers_run_trn() ([]byte, error) {
	return bindata_read([]byte{
		,
		},
		"gatherers/run-trn",
	)
}

func gatherers_run_wbd() ([]byte, error) {
	return bindata_read([]byte{
		0x1f, 0x8b, 0x08, 0x00, 0x00, 0x09, 0x6e, 0x88, 0x00, 0xff, 0x6c, 0x8d,
		0x41, 0x0e, 0xc2, 0x30, 0x0c, 0x04, 0xef, 0xbc, 0xc2, 0xf2, 0x07, 0xfc,
		0x24, 0x44, 0xa8, 0x53, 0x2c, 0x15, 0xb6, 0xb2, 0x5b, 0x25, 0xcf, 0x27,
		0x22, 0xe4, 0x50, 0x29, 0x17, 0xcf, 0x5c, 0x3c, 0xbb, 0x98, 0xdf, 0xb5,
		0x5a, 0x1c, 0x41, 0x2c, 0xd8, 0x0f, 0x59, 0x81, 0x75, 0x53, 0x79, 0xbe,
		0x1c, 0xef, 0x01, 0x26, 0xee, 0x42, 0x45, 0x13, 0x25, 0x47, 0x09, 0x75,
		0xca, 0x38, 0x3f, 0x0b, 0xdf, 0x2e, 0x85, 0x33, 0x5c, 0x36, 0x4b, 0x92,
		0xcd, 0x35, 0xa3, 0x0e, 0xb6, 0xc0, 0xdf, 0x26, 0x05, 0x9a, 0x27, 0xb0,
		0xab, 0x3f, 0xfa, 0x6d, 0xef, 0x3f, 0xce, 0xe6, 0xbf, 0x01, 0x00, 0x00,
		0xff, 0xff, 0x1e, 0x6b, 0x0f, 0xdf, 0xc1, 0x00, 0x00, 0x00,
		},
		"gatherers/run-wbd",
	)
}

func gatherers_run_zsc() ([]byte, error) {
	return bindata_read([]byte{
		0x1f, 0x8b, 0x08, 0x00, 0x00, 0x09, 0x6e, 0x88, 0x00, 0xff, 0x4a, 0x4d,
		0xce, 0xc8, 0x57, 0x50, 0xaa, 0x2a, 0xce, 0x28, 0x4a, 0x56, 0xc8, 0xc9,
		0xcc, 0x4b, 0x55, 0x48, 0xce, 0x2f, 0xcd, 0x2b, 0x51, 0x52, 0x50, 0xf1,
		0x0b, 0xf5, 0x75, 0x72, 0x0d, 0x52, 0x50, 0xd1, 0xc8, 0x2b, 0xcd, 0x4d,
		0x4a, 0x2d, 0x8a, 0xcf, 0x4f, 0x8b, 0x07, 0xc9, 0x17, 0x2b, 0xd4, 0xe9,
		0xeb, 0x81, 0x95, 0x6b, 0x72, 0x01, 0x02, 0x00, 0x00, 0xff, 0xff, 0x96,
		0x53, 0x80, 0xcd, 0x3c, 0x00, 0x00, 0x00,
		},
		"gatherers/run-zsc",
	)
}


// Asset loads and returns the asset for the given name.
// It returns an error if the asset could not be found or
// could not be loaded.
func Asset(name string) ([]byte, error) {
	if f, ok := _bindata[name]; ok {
		return f()
	}
	return nil, fmt.Errorf("Asset %s not found", name)
}

// _bindata is a table, holding each asset generator, mapped to its name.
var _bindata = map[string] func() ([]byte, error) {
	"gatherers/common": gatherers_common,
	"gatherers/run-abu": gatherers_run_abu,
	"gatherers/run-bac": gatherers_run_bac,
	"gatherers/run-crd": gatherers_run_crd,
	"gatherers/run-dbd": gatherers_run_dbd,
	"gatherers/run-dmp": gatherers_run_dmp,
	"gatherers/run-dod": gatherers_run_dod,
	"gatherers/run-esi": gatherers_run_esi,
	"gatherers/run-fic": gatherers_run_fic,
	"gatherers/run-hii": gatherers_run_hii,
	"gatherers/run-moi": gatherers_run_moi,
	"gatherers/run-myi": gatherers_run_myi,
	"gatherers/run-nus": gatherers_run_nus,
	"gatherers/run-poi": gatherers_run_poi,
	"gatherers/run-psi": gatherers_run_psi,
	"gatherers/run-sii": gatherers_run_sii,
	"gatherers/run-trn": gatherers_run_trn,
	"gatherers/run-wbd": gatherers_run_wbd,
	"gatherers/run-zsc": gatherers_run_zsc,

}
